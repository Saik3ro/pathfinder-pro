                            ...
                            onChange={async (e) => {
                              ...
                              if (existing) {
                                ...
                                if (!error) {
                                  ...
                                  // Update learning_decks progress_percentage
                                  const newDone = isCompleted ? done + 1 : done - 1;
                                  const newPct = milestones.length ? Math.round((newDone / milestones.length) * 100) : 0;
                                  await supabase
                                    .from("learning_decks")
                                ...
                                if (!error && data) {
                                  ...
                                  // Update learning_decks progress_percentage
                                  const newDone = isCompleted ? done + 1 : done;
                                  const newPct = milestones.length ? Math.round((newDone / milestones.length) * 100) : 0;
                                  await supabase
                                    .from("learning_decks")
                                    .update({ progress_percentage: newPct })
